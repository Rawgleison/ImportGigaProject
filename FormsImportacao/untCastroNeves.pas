...
var
  frmCastroNeves: TfrmCastroNeves;
  ArqName: string;
...
procedure TfrmCastroNeves.CLIENTES;
var
...
procedure TfrmCastroNeves.FORNECEDORES;
var
...
procedure TfrmCastroNeves.butGravarClick(Sender: TObject);
begin
    vleConexao.Strings.SaveToFile(ArqName);
end;

procedure TfrmCastroNeves.GRUPOS_PRODUTOS;
begin
  with DM do
  begin
    ConectaGiga;
...
procedure TfrmCastroNeves.MARCAS;
begin
  with DM do
  begin
...
procedure TfrmCastroNeves.PESSOAS;
var
...
procedure TfrmCastroNeves.PRODUTOS;
var
...
//  ShowMessage('Testando os 100 primeiros');
  DM.qrImport.Open('SELECT * FROM IMPORT_PRODUTO');
  dm.qrImport.Last;
  dm.qrImport.First;
...
//    PROD.PRODUTOS.COD_PRODUTO:=IntToStr(I);
    PROD.PRODUTOS.COD_PRODUTO := dm.qrImport.FieldByName('CODPROD').AsString;
    PROD.PRODUTOS_APELIDOS_PROD[3].APELIDO := dm.qrImport.FieldByName('CODIGOINTERNO').AsString;
    PROD.PRODUTOS.DESC_PRODUTO := dm.qrImport.FieldByName('NOMEREAL').AsString;
    PROD.PRODUTOS.DESC_PRODUTO_FISCAL := dm.qrImport.FieldByName('NOMENOTA').AsString;
    PROD.PRODUTOS_GRADE_UND[1].UNIDADE := dm.qrImport.FieldByName('UNIDADEVENDA').AsString;
    PROD.PRODUTOS_TAB_PRECOS_PROD[1].PRECO := dm.qrImport.FieldByName('PRECOVENDA').AsFloat;
    PROD.PRODUTOS_TAB_PRECOS_PROD[2].PRECO := dm.qrImport.FieldByName('PRECOTABELA1').AsFloat;
    PROD.PRODUTOS_CUSTO[1].PRECO_CUSTO := dm.qrImport.FieldByName('PRECOCUSTO').AsFloat;
         if dm.qrImport.FieldByName('SITUACAOTRIBUTARIA').AsString = '010' then PROD.PRODUTOS.COD_IMPOSTO := 2
    else if dm.qrImport.FieldByName('SITUACAOTRIBUTARIA').AsString = '040' then PROD.PRODUTOS.COD_IMPOSTO := 3
    else if dm.qrImport.FieldByName('SITUACAOTRIBUTARIA').AsString = '060' then PROD.PRODUTOS.COD_IMPOSTO := 4
    else if dm.qrImport.FieldByName('SITUACAOTRIBUTARIA').AsString = '102' then PROD.PRODUTOS.COD_IMPOSTO := 5
    else PROD.PRODUTOS.COD_IMPOSTO := 1;
    PROD.PRODUTOS_APELIDOS_PROD[2].APELIDO := dm.qrImport.FieldByName('CODBARRAS').AsString;
    if Trim(dm.qrImport.FieldByName('BALANCA').AsString) <> '' then
      PROD.PRODUTOS.FLAG_PESO := 1
    else
      PROD.PRODUTOS.FLAG_PESO := 0;
...
procedure TfrmCastroNeves.butImportClick(Sender: TObject);
begin
...
procedure TfrmCastroNeves.Button1Click(Sender: TObject);
var
 s: TArray<string>;
 i: string;
begin
...
procedure TfrmCastroNeves.Button2Click(Sender: TObject);
begin
  if OpenDialog1.Execute then
    vleConexao.Values['Database']:=OpenDialog1.FileName;
end;

procedure TfrmCastroNeves.TRANSPORTADORAS;
var
...
procedure TfrmCastroNeves.ComboBox1Change(Sender: TObject);
begin
...
procedure TfrmCastroNeves.ConectaGiga;
begin
...
procedure TfrmCastroNeves.FormCreate(Sender: TObject);
begin
  Label1.Caption:='';
end;

procedure TfrmCastroNeves.FormShow(Sender: TObject);
begin
